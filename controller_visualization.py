...
def calculate_avg_vacuum_pressure(polygon_obj, suction_cup):
    ...
    avg_vacuum_pressure = np.mean(vacuum_pressures)
    return avg_vacuum_pressure
...
if __name__ == "__main__":
    ...
    suction_cup_center = [-10, -10] # dumbbell shape
    suction_cup = SuctionCup(num_chambers, suction_cup_radius, suction_cup_center, initial_yaw_angle)
    
    # Record the average pressure each (x, y) point   
    avg_pressure = []
    for x in np.arange(-10, 40, step_size):
        for y in np.arange(-10, 30, step_size):
            suction_cup.center = [x, y]
            suction_cup.chambers = suction_cup.create_chambers()
            avg_pressure.append([x, y, calculate_avg_vacuum_pressure(polygon_obj, suction_cup)])
    
    # Convert data to numpy array for easier handling
    avg_pressure = np.array(avg_pressure)

    # Extract x, y, and z values for plotting
    x_vals = avg_pressure[:, 0]
    y_vals = avg_pressure[:, 1]
    z_vals = -np.log10(avg_pressure[:, 2])

    # Create 3D plot
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    
    # Plot the polygon in 3D
    xx, yy = polygon_obj.polygon.exterior.coords.xy
    zz = np.zeros_like(xx) + 1  # Assuming the polygon is in the XY plane
    ax.plot(xx, yy, zz, color='r', label='Polygon', linewidth=2)

    ax.scatter(x_vals, y_vals, z_vals, c=z_vals, cmap='viridis', label='Vacuum Pressure')

    # Add labels and title
    ax.set_xlabel('X coordinate')
    ax.set_ylabel('Y coordinate')
    ax.set_zlabel('Average Pressure (log10)')
    ax.set_title('3D Graph of Average Vacuum Pressure and Polygon')
    ax.legend()
    
    
    # 2D plot for the polygon
    fig2 = plt.figure()
    ax1 = fig2.add_subplot(111)
    xx, yy = polygon_obj.polygon.exterior.coords.xy
    # ax1.plot(xx, yy, 'r-', label='Polygon')
    polygon_obj.draw(ax1)
    ax1.set_xlabel('X coordinate')
    ax1.set_ylabel('Y coordinate')
    # ax1.set_title('2D Polygon Plot')
    ax1.legend()
    ax1.grid(True)
