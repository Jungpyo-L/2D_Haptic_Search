...
def animate_suction_cup(i, ax, polygon_obj, suction_cup, step_size=1):
    global haptic_path  # Use global variable for haptic_path
    ...
    # Update global haptic_path
    haptic_path = np.vstack((haptic_path, suction_cup.center))
    
    plot_polygon_and_suction_cup(ax, polygon_obj, suction_cup)
    
# def plot_haptic_path(polygon_obj, suction_cup, haptic_path, label):
def plot_haptic_path(ax, haptic_path):
    polygon_obj.draw(ax)
    # suction_cup.draw(ax)
    colors = ['r', 'g', 'b']
...
def calculate_path_length(haptic_path):
    ...
    # Sum up the segment lengths to get the total path length
...
if __name__ == "__main__":
    ...
    # Define haptic search parameters
    step_size = 0.5  # in mm
    
    # Initialize figure for plotting haptic paths
    fig, ax = plt.subplots(figsize=(5, 4))
    
    # List to hold haptic paths for different chamber configurations
    haptic_paths = []
    ...
    for num_chambers in num_chambers_list:
        ...
        # Initialize global haptic_path with the initial center
        global haptic_path
        haptic_path = np.array([suction_cup.center])
        
        # Create plot and animate the suction cup movement
        fig_anim, ax_anim = plt.subplots(figsize=(5, 4))
        ani = FuncAnimation(fig_anim, animate_suction_cup, fargs=(ax_anim, polygon_obj, suction_cup, step_size), interval=10, repeat=False)
        
        # Save the animation
        ani.save(f'suction_cup_animation_{num_chambers}_chambers.gif', writer='imagemagick', fps=15)  # for gif
        
        # Store the haptic path
        # haptic_paths.append((num_chambers, haptic_path))
        haptic_paths.append(haptic_path)
        
    # Plot all haptic paths together
    plot_haptic_path(ax, haptic_paths)
    
    # Plot all haptic paths together
    plt.show()
    
     # Calculate and print the haptic path length for each configuration
