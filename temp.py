...
def calculate_unit_vectors(num_chambers, rotation_angle):
    angles = np.linspace(0, 2 * np.pi, num_chambers, endpoint=False) + np.radians(rotation_angle)
...
def haptic_search_suction_cup(i, ax, polygon_obj, suction_cup, step_size, haptic_path, controller, center_yaw_history, animation=False):
    if check_suction(polygon_obj, suction_cup):
        if animation:
            plot_polygon_and_suction_cup(ax, polygon_obj, suction_cup)
        return
    
    damping_factor = 0.5  # Define the damping factor
    max_velocity_x = 0.5  # Define the maximum velocity limit for the x direction
    max_velocity_y = 0.5  # Define the maximum velocity limit for the y direction
...
def save_animation(fig, axs, polygon_objs, suction_cups, step_size, haptic_paths, center_yaw_histories, filename, output_folder, controller):
    def update(i):
        for ax, polygon_obj, suction_cup, haptic_path, center_yaw_history in zip(axs.flat, polygon_objs, suction_cups, haptic_paths, center_yaw_histories):
            if i < len(center_yaw_history):
                ...
                suction_cup.chambers = suction_cup.create_chambers()
                plot_polygon_and_suction_cup(ax, polygon_obj, suction_cup)
    ani = FuncAnimation(fig, update, frames=len(center_yaw_histories[0]), repeat=False)
...
def haptic_search_task(args):
    i, ax, polygon_obj, suction_cup, step_size, haptic_path, center_yaw_history, controller = args
    haptic_search_suction_cup(i, ax, polygon_obj, suction_cup, step_size, haptic_path, center_yaw_history, controller, animation=False)

def main(args):
    num_chambers_list = args.num_chambers_list
    step_size = 0.05 if 'momentum' in args.controller else 0.5
    ...
    for num_chambers in num_chambers_list:
        ...
        if args.animation:
            ...
            save_animation(fig, axs, polygon_objs, suction_cup_animation[num_chambers], step_size, haptic_paths[num_chambers], center_yaw_histories[num_chambers], f'suction_cup_animation_{num_chambers}_chambers_'+ args.controller +'.gif', output_folder, args.controller)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_polygons', type=int, help='number of polygons to generate', default=4)
